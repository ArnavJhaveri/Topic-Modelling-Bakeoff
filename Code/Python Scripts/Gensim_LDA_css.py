...
for run in range(1, 6):
    ...
    try:
        os.makedirs(folder + 'GensimLDA/run_' + str(run))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    ...
    import json
    with open(folder + 'GensimLDA/run_' + str(run) + '/hyperparams.txt', 'w') as f:
        f.write(json.dumps(hyperparams))

    # write stopwords to file
    with open(folder + 'GensimLDA/run_' + str(run) + '/stopwords.txt', 'w') as f:
    ...
    for num_topics in num_topics_list:
        ...
        #         r2s.append(r2_score(dummy_test[label_col], lr_model.predict(X_test)))
            
        #     accuracies = [np.mean(accuracies), lda_topic_diversity, lda_coherence] + accuracies + [np.mean(r2s)]
        #     final_df = pd.concat([pd.Series(accuracies)], axis = 1).T
        #     final_df.columns = ['Average Accuracy', 'Topic Diversity', 'Topic Coherence'] + [i + str(" Accuracy") for i in dummy_train.columns] + ['R2']
        ...
        with open(folder + 'GensimLDA/run_' + str(run) + '/topics_' + str(num_topics) + '.txt', 'w') as f:
        ...
        with open(folder + 'GensimLDA/run_' + str(run) + '/labels.pkl', 'wb') as f:
            pickle.dump(labels, f)
        
        with open(folder + 'GensimLDA/run_' + str(run) + '/train.pkl', 'wb') as f:
            pickle.dump(train, f)
        
        with open(folder + 'GensimLDA/run_' + str(run) + '/test.pkl', 'wb') as f:
            pickle.dump(test, f)
        
        with open(folder + 'GensimLDA/run_' + str(run) + '/lda_loglik.pkl', 'wb') as f:
            pickle.dump(lda_loglik, f)
        ...
        if folder[:-1] in ['fb', 'qualtrics', 'twitter', 'facebook_2020']:
            ...
            with open(folder + 'GensimLDA/run_' + str(run) + '/GensimLDA_topic_distribution_test.pkl', 'wb') as f:
                pickle.dump(X_test, f)
            
            with open(folder + 'GensimLDA/run_' + str(run) + '/GensimLDA_topic_distribution_train.pkl', 'wb') as f:
                pickle.dump(X, f)
