...
label_column = 'age'
id_column = 'Unnamed: 0'
dataset_path = 'data/facebook_2020.csv'
folder = 'facebook_2020/'
...
for run in range(1, 6):
    ...
    try:
        os.makedirs(folder + 'CTM/run_' + str(run))
    except OSError as e:
        if e.errno != errno.EEXIST:
            raise
    ...
    import json
    with open(folder + 'CTM/run_' + str(run) + '/hyperparams.txt', 'w') as f:
        f.write(json.dumps(hyperparams))

    # write stopwords to file
    with open(folder + 'CTM/run_' + str(run) + '/stopwords.txt', 'w') as f:
        for item in mallet_stopwords:
            f.write(item + ", ")

    with open(folder + 'CTM/run_' + str(run) + '/labels.pkl', 'wb') as f:
            pickle.dump(labels, f)
        
    with open(folder + 'CTM/run_' + str(run) + '/train.pkl', 'wb') as f:
        pickle.dump(train, f)
    
    with open(folder + 'CTM/run_' + str(run) + '/test.pkl', 'wb') as f:
        pickle.dump(test, f)    
    ...
    for num_topics in num_topics_list:
        ...
        with open(folder + 'CTM/run_' + str(run) + '/topics_' + str(num_topics) + '.txt', 'w') as f:
        ...
        if folder[:-1] not in ['fb', 'qualtrics', 'twitter', 'facebook_2020']:
            with open(folder + 'CTM/run_' + str(run) + '/CTM_topic_distribution_train.pkl', 'wb') as f:
                pickle.dump(ctm_topic_distribution_train, f)
        ...
        if folder[:-1] not in ['fb', 'qualtrics', 'twitter', 'facebook_2020']:
            with open(folder + 'CTM/run_' + str(run) + '/CTM_topic_distribution_test.pkl', 'wb') as f:
                    pickle.dump(ctm_topic_distribution_test, f)
        ...
        if folder[:-1] in ['fb', 'qualtrics', 'twitter', 'facebook_2020']:
            model = 'CTM'
            ...
            train_distribution = X[:round(0.80 * len(X))]
