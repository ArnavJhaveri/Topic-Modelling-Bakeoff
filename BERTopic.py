...
import json
with open(folder + 'BERTopic/hyperparams.txt', 'w') as f:
  f.write(json.dumps(hyperparams))

# write stopwords to file
with open(folder + 'BERTopic/stopwords.txt', 'w') as f:
...
for num_topics in num_topics_list:
    ...
    bert_r2_train, bert_mae_train, bert_mse_train, bert_rmse_train, bert_r2_test, bert_mae_test, bert_mse_test, bert_rmse_test, bert_regression = results(bert_topic_topics, bert_topic_loglik)

    bert_topic_diversity = proportion_unique_words(bert_topic_topics)

    tokenizer = lambda s: re.findall( '\w+', s.lower() )
    texts = [tokenizer(t) for t in  documents]
    bert_coherence = CoherenceNPMI(texts=texts, topics=bert_topic_topics).score()
    
    final_df = pd.concat([pd.Series([bert_r2_train, bert_mae_train, bert_mse_train, bert_rmse_train, bert_r2_test, bert_mae_test, bert_mse_test, bert_rmse_test, bert_topic_diversity, bert_coherence])], axis = 1).T
    ...
    with open(folder + 'BERTopic/LR_coefficients_' + str(num_topics) + '.txt', 'w') as f:
        for item in bert_regression.coef_:
            f.write("%s\n" % item)
    
    final_df.to_csv(folder + 'BERTopic/df_' + str(num_topics) + '.csv')

    with open(folder + 'BERTopic/topics_' + str(num_topics) + '.txt', 'w') as f:
