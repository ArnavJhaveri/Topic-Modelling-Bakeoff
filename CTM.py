...
def results(ctm_topic_distribution_train, ctm_topic_distribution_test, flag=False):  
  """
    PARAMS
        ctm_topic_distribution_train: topic distribution in training set
        ctm_topic_distribution_test: topic distribution in training set
...
import json
with open(folder + 'CTM/hyperparams.txt', 'w') as f:
  f.write(json.dumps(hyperparams))

# write stopwords to file
with open(folder + 'CTM/stopwords.txt', 'w') as f:
...
for num_topics in num_topics_list:
  ...
  warnings.filterwarnings('ignore')
  ctm = CombinedTM(bow_size=len(tp.vocab), contextual_size=768, n_components=num_topics, num_epochs=num_epochs_ctm)
  ...
  warnings.filterwarnings('ignore')
  testing_dataset = tp.transform(text_for_contextual=test_unpreprocessed_corpus, text_for_bow=test_preprocessed_documents)
  ...
  ctm_r2_train, ctm_mae_train, ctm_mse_train, ctm_rmse_train, ctm_r2_test, ctm_mae_test, ctm_mse_test, ctm_rmse_test, ctm_regression = results(ctm_topic_distribution_train, ctm_topic_distribution_test, True)

  ctm_topic_diversity = proportion_unique_words(ctm_topics)

  tokenizer = lambda s: re.findall( '\w+', s.lower() )
  texts = [tokenizer(t) for t in  documents]
  ctm_coherence = CoherenceNPMI(texts=texts, topics=ctm_topics).score()
  
  final_df = pd.concat([pd.Series([ctm_r2_train, ctm_mae_train, ctm_mse_train, ctm_rmse_train, ctm_r2_test, ctm_mae_test, ctm_mse_test, ctm_rmse_test, ctm_topic_diversity, ctm_coherence])], axis = 1).T
  ...
  with open(folder + 'CTM/LR_coefficients_' + str(num_topics) + '.txt', 'w') as f:
        for item in ctm_regression.coef_:
            f.write("%s\n" % item)
    
  final_df.to_csv(folder + 'CTM/df_' + str(num_topics) + '.csv')

  with open(folder + 'CTM/topics_' + str(num_topics) + '.txt', 'w') as f:
